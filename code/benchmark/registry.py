...
Usage:
    1. Auto-export decorator (RECOMMENDED - handles deep hierarchies):
       ```python
       from benchmark.registry import export_benchmark
    ...
    2. Import check at module level:
       ```python
       from benchmark.registry import require_get_benchmark
    ...
    4. Validation script (run in CI):
       ```bash
       python -m benchmark.registry --validate
       ```
"""
...
def main():
    ...
    if args.validate:
        repo_root = args.repo_root if args.repo_root else Path(__file__).parent.parent
